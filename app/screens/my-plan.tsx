...
const MyPlanScreen = ({ navigation }: Props) => {
    ...
    const fetchData = async () => {
      try {
        ...
        supabase
          ...
          .then(({ data, error }) => {
            if (error) {
              throw new Error(`Error fetching common allergies: ${error.message}`);
            }
  ...
  const saveChanges = async () => {
    ...
    selectedDiets.map(async (diet) => {
      const { error } = await supabase.from('user_diets').insert({ diet_name: diet });
    ...
    selectedAllergies.map(async (allergy) => {
      const { error } = await supabase.from('user_allergies').insert({ allergy_name: allergy });
      if (error) console.error('insert error', error);
      return;
    });
  };
        ...
        <View className="flex-wrap flex-row gap-2 mt-0">
          {commonAllergies?.map((allergy) => (
            <View key={allergy}>
              ...
              >
                <Text className={` ${selectedAllergies.includes(allergy) && 'bg-[#5e9e38] text-white'}
`}>
